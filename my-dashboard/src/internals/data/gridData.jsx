...
function renderStatus(status) {
  ...
  const label = {
    Basic: 'Запись',
    Advanced: 'Запись + Внески',
    Ultra: 'Cопровождение',
    Others: 'Другие услуги',
  }

  return <Chip label={label[status] ?? "Нет Услуги"} color={colors[status] ?? "primary"} size="small" />;
}
...
function renderDate(date) {
  ...
  return <label>{`${day}/${month}/${year} (${hours}:${minutes})`}</label>;
}
...
export const columns = [
  { field: 'name', headerName: 'Имя', flex: 1.5, minWidth: 120 },
  {
    field: 'surname',
    headerName: 'Фамилия',
    headerAlign: 'left',
